...
-(id)initWithViewController:(UIViewController*)viewController
{
    ...
    if(self)
    {
        ...
        FPPopoverController *__weak weak = self;
        [_touchView setTouchedOutsideBlock:^{
            [weak dismissPopoverAnimated:YES];
        }];
...
-(CGPoint)originFromView:(UIView*)fromView fromRect:(CGRect)fromRect
{
    CGPoint p = {0, 0};
    if([_contentView arrowDirection] == FPPopoverArrowDirectionUp)
    {
        p.x = fromRect.origin.x + fromRect.size.width/2.0;
        p.y = fromRect.origin.y + fromRect.size.height;
    }
    else if([_contentView arrowDirection] == FPPopoverArrowDirectionDown)
    {
        p.x = fromRect.origin.x + fromRect.size.width/2.0;
        p.y = fromRect.origin.y;
    }
    else if([_contentView arrowDirection] == FPPopoverArrowDirectionLeft)
    {
        p.x = fromRect.origin.x + fromRect.size.width;
        p.y = fromRect.origin.y + fromRect.size.height/2.0;
    }
    else if([_contentView arrowDirection] == FPPopoverArrowDirectionRight)
    {
        p.x = fromRect.origin.x;
        p.y = fromRect.origin.y + fromRect.size.height/2.0;
    }
...
- (void)presentPopoverFromRect:(CGRect)rect inView:(UIView *)view
{
    _fromView = view;
    _fromRect = rect;
    [self presentPopoverFromPoint:[self originFromView:_fromView fromRect:rect]];
}

- (void)presentPopoverFromRect:(CGRect)rect inView:(UIView *)view inRect:(CGRect)bound
{
    _fromView = view;
    _fromRect = rect;
    [self presentPopoverFromPoint:[self originFromView:_fromView fromRect:rect]];
...
{
    _fromRect = rect;
    CGPoint fromPoint = [self originFromView:_fromView fromRect:rect];
    self.origin = fromPoint;
    _contentView.relativeOrigin = [_parentView convertPoint:fromPoint toView:_contentView];
...
-(CGRect)bestArrowDirectionAndFrameFromView:(UIView*)v
{
    CGPoint p = [v convertPoint:_fromRect.origin toView:self.view];
    
    CGRect rect = _fromRect;
    if (CGRectIsEmpty(rect)) {
        rect = _fromView.frame;
    }
    
    CGFloat ht = p.y; //available vertical space on top of the view
    CGFloat hb = [self parentHeight] -  (p.y + rect.size.height); //on the bottom
    CGFloat wl = p.x; //on the left
    CGFloat wr = [self parentWidth] - (p.x + rect.size.width); //on the right
    ...
    CGRect r;
    r.size = [_contentView sizeFromContentSize:_contentSize];
    
    if (r.size.width > [self parentWidth]) {
        r.size.width = [self parentWidth];
    }
    if (r.size.height > [self parentHeight]) {
        r.size.height = [self parentHeight];
    }
    
    if (bestDirection == FPPopoverArrowDirectionUp) {
        r.origin.x = p.x + rect.size.width/2.0 - r.size.width/2.0;
        r.origin.y = p.y + rect.size.height;
    }
    else if (bestDirection == FPPopoverArrowDirectionDown) {
        r.origin.x = p.x + rect.size.width/2.0 - r.size.width/2.0;
        r.origin.y = p.y - r.size.height;
    }
    else if (bestDirection == FPPopoverArrowDirectionLeft) {
        r.origin.x = p.x + rect.size.width;
        r.origin.y = p.y + rect.size.height/2.0 - r.size.height/2.0;
    }
    else if (bestDirection == FPPopoverArrowDirectionRight) {
